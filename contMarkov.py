...
class contMarkov(object): #This class will contain various methods relevant to continuous time Markov chain simulations
    ...
    def multCTMC(self): #This method will allow me to run many continuous Markov simulations and store the results as a list of lists
        for int in range(self.numSims):
            self.multChains.append(self.contMarkovSim()) 
        return self.multChains
                
   
    def optimizeBrl(self, BrlCurr=0.5, diff=0.1,thresh=0.001): #This function is very similar to my earlier optimization function. Goal: find most likely branchlength given a starting and ending state.
        while diff > thresh: #This while statement limits the resolution of the optimization (and prevents getting stuck in the second while loop)
            ...
            while Lik_BrlCurr < Lik_BrlUp or Lik_BrlCurr < Lik_BrlDown:   
                if Lik_BrlUp > Lik_BrlCurr:         
                    BrlCurr = BrlUp #change value by step diff each time the "if" statement is met                
                elif Lik_BrlDown > Lik_BrlCurr: #this "while" section decreases each
                    BrlCurr = BrlDown #change value by step diff each time the "if" statement is met
                BrlUp = BrlCurr + diff            
                BrlDown = BrlCurr - diff  
                if BrlDown < 0: 
                    BrlDown = 0 
        ...
        return BrlCurr, Lik_BrlCurr
    
    def mulOptimizeBrl(self, BrlCurr=0.9, diff=0.1, thresh=0.001):
        while diff > thresh: #This while statement limits the resolution of the optimization (and prevents getting stuck in the second while loop)
            firstStates = []
            lastStates = []            
            for cs in self.multChains: #Earlier I was troubleshooting with the fixed sites variable below (same sites that Glaucia used)                                        
            #sites = [[0,1,3,1,2,0],[1,2,1,3,2,3,0,2,0],[0,3,2,3,1],[1,0,2,1],[3,2,1,3],[2,1,2,3,1,2],[1,3,2,0,2,1],[2,3,1,0,2,0],[0,1,2,3,0,1]]
            #for cs in sites: #Will change "sites" to self.multChains once I have the method functioning properly 
                first = cs[0]
                last = cs[-1] 
            ...
            downLikList = []
            upLikList = []
            for f,l in zip (firstStates,lastStates): #Thanks to Glaucia for this line's syntax
                Lik_BrlCurr1 = self.allMargProb(chainLength = BrlCurr)[f][l]
                currLikList.append(Lik_BrlCurr1)
            #print currLikList
            for f,l in zip (firstStates,lastStates):
                Lik_BrlDown1 = self.allMargProb(chainLength = BrlDown)[f][l]
                downLikList.append(Lik_BrlDown1)
            for f,l in zip (firstStates,lastStates):
                Lik_BrlUp1 = self.allMargProb(chainLength = BrlUp)[f][l]
                upLikList.append(Lik_BrlUp1)
            Lik_BrlCurr= functools.reduce(operator.mul, currLikList, 1)#Multiplying all likelihood values together in list
            Lik_BrlDown= functools.reduce(operator.mul, downLikList, 1)#Multiplying all likelihood values together in list
            Lik_BrlUp= functools.reduce(operator.mul, upLikList, 1)#Multiplying all likelihood values together in list
            while Lik_BrlCurr < Lik_BrlUp or Lik_BrlCurr < Lik_BrlDown:   
                if Lik_BrlUp > Lik_BrlCurr:         
                    BrlCurr = BrlUp #change value by step diff each time the "if" statement is met                
                elif Lik_BrlDown > Lik_BrlCurr: #this "while" section decreases each
                ...
                if BrlDown < 0: 
                    BrlDown = 0 
                currLikList = [] #Need to reset list
                downLikList = []#Need to reset list
                upLikList = []#Need to reset list
                for f,l in zip (firstStates,lastStates): #Thanks to Glaucia for this line's syntax
                    Lik_BrlCurr1 = self.allMargProb(chainLength = BrlCurr)[f][l]
                    currLikList.append(Lik_BrlCurr1)
                for f,l in zip (firstStates,lastStates):
                ...
                Lik_BrlDown= functools.reduce(operator.mul, downLikList, 1)#Multiplying all likelihood values together in list
                Lik_BrlUp= functools.reduce(operator.mul, upLikList, 1)#Multiplying all likelihood values together in list
            diff *= 0.5 #Here I want to hone the value so I reduce diff by half 
        return BrlCurr, Lik_BrlCurr
...
print d.mulOptimizeBrl(BrlCurr = 5) 
print d.mulOptimizeBrl(BrlCurr = 0.3) 
print d.mulOptimizeBrl(BrlCurr = 1) 
